...
public class SoundManager : MonoBehaviour
{
	...
	[SerializeField] private string[] RoundEndSounds =
	{
		"ApcDestroyed",
		"BanginDonk",
		"Disappointed",
		...
		float pan = 0)
	{
		...
		var sound = AudioSourcePool.Instance.GetSourceFromPool(Instance.sounds[name]);
		if (pitch > 0)
		{
	...
	{
		name = Instance.ResolveSoundPattern(name);
		Instance.PlaySource(AudioSourcePool.Instance.GetSourceFromPool(Instance.sounds[name]));
	}
	...
	public static void Stop(string name)
	{
		if (Instance.sounds.ContainsKey(name))
		{
			...
			for (int i = AudioSourcePool.Instance.pooledSources.Count - 1; i > 0; i--)
			{
				if (AudioSourcePool.Instance.pooledSources[i] == null) continue;

				if (AudioSourcePool.Instance.pooledSources[i].isPlaying && AudioSourcePool.Instance.pooledSources[i].audioSource.clip == sound.clip)
				{
					AudioSourcePool.Instance.pooledSources[i].audioSource.Stop();
				}
			}
		...
		else
		{
			foreach (var sound in Instance.GetMatchingSounds(name))
			{
				...
				{
					if (AudioSourcePool.Instance.pooledSources[i] == null) continue;
					if (AudioSourcePool.Instance.pooledSources[i].isPlaying
					    && AudioSourcePool.Instance.pooledSources[i].audioSource.clip == s.clip)
					{
						AudioSourcePool.Instance.pooledSources[i].audioSource.Stop();
					}
				}
