...
public class SoundManager : MonoBehaviour
{
		...
		float pan = 0)
	{
		...
		var sound = SoundSpawn.Instance.GetSourceFromPool(Instance.sounds[name]);
		if (pitch > 0)
		{
	...
	{
		name = Instance.ResolveSoundPattern(name);
		Instance.PlaySource(SoundSpawn.Instance.GetSourceFromPool(Instance.sounds[name]));
	}
	...
	public static void Stop(string name)
	{
		...
		{
			var audioSource = Instance.sounds[name];

			SoundSpawn.Instance.Stop(audioSource);
		}
		else
		{
			foreach (var sound in Instance.GetMatchingSounds(name))
			{
				var audioSource = Instance.sounds[sound];
				SoundSpawn.Instance.Stop(audioSource);
				audioSource.Stop();
			}
		}
