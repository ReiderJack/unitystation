...
public class LightMountStatesV2 : NetworkBehaviour, ICheckedInteractable<HandApply>
{
	...
	private LightSourceV2 lightSource;
	...
	public void ServerPerformInteraction(HandApply interaction)
	{
		...
		else if (Validations.HasItemTrait(interaction.HandObject, traitRequired) && state == LightMountState.MissingBulb)
		{
			...
			else
			{
				...
				if (lightSource.lightState == LightState.On)
				{
					Despawn.ServerSingle(interaction.HandObject);
	...
	bool EnsureInit()
	{
		if (lightSource == null) lightSource = GetComponent<LightSourceV2>();
